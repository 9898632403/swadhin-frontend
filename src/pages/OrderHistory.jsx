...
const OrderHistory = () => {
  ...
  const generateBill = (order) => {
    const doc = new jsPDF();
    doc.setFontSize(18).text("SWADHIN - Invoice/Bill", 20, 20);
    doc.setFontSize(12);
    ...
    if (order.coupon_code) doc.text(`Coupon Applied: ${order.coupon_code}`, 20, y + 20);
    doc.setFontSize(11).text("Note: Your order will be delivered in 7-8 days.", 20, y + 35);
                ...
                {order.items?.map((item, idx) => (
                  <li key={idx} className="orderItem">
                    <Link to={`/product/${item.id || item._id || item.productId}`} className="productLink">
                      <img src={item.image} alt={item.name} className="productThumb" loading="lazy" />
